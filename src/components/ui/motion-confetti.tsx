...
type ConfettiProps = {
  ...
  /** Hide the built-in Celebrate button so another control can call fire(). */
  showTrigger?: boolean
  /** Scatter papers across the viewport instead of one point in the center. */
  fullscreen?: boolean
}
...
function ParticleDot({ particle }: { particle: Particle }) {
  ...
  const { keyframes, duration, size, color, shape, originX, originY } = particle
...
export const Confetti = forwardRef<ConfettiHandle, ConfettiProps>(function Confetti(
  ...
  const fire = useCallback(() => {
    ...
    const particles = Array.from({ length: particleCount }, () => {
      ...
      const originX = fullscreen ? `${Math.random() * 100}%` : undefined
      const originY = fullscreen
        ? `${(Math.random() < 0.6 ? Math.random() * 40 : Math.random() * 100)}%`
        : undefined
      return {
        originX,
        originY,
  ...
  }, [particleCount, startVelocity, spread, decay, gravity, drift, duration, size, fullscreen])

  useImperativeHandle(ref, () => ({ fire }), [fire])

  return (
    <div className={fullscreen ? "confetti-stage confetti-stage--full" : "confetti-stage"}>
